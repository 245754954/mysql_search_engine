 ...
 * @return 分割出来的词元的长度
 */
static int wiser_isalpha(const UTF32Char ustr)
{

  if (('A' <= ustr && ustr <= 'Z') || ('a' <= ustr && ustr <= 'z'))
  {
    return 1;
  }
  else
...
ngram_next(const UTF32Char *ustr, const UTF32Char *ustr_end,
           unsigned int n, const UTF32Char **start, const UTF32Char **next)
{
  int i;
  const UTF32Char *p;
                           ...
                           inverted_index_hash **postings)
{
  ...
  char out[BUFFER_SIZE_DOC_CONTENT] = {'\0'};
  md5_without_salt(&env->ctx, (char *)token, out);
  // printf("md5 is %s\n",out);
  // token_id = db_get_token_id1(env, token, token_size, document_id, &token_docs_count);
  token_id = db_get_token_id1(env, out, strlen(out), document_id, &token_docs_count);
  if (*postings)
  {
...
void dump_token(wiser_env *env, int token_id)
{
  ...
  db_get_token1(env, token_id, token, &token_len);
