...
start(void *user_data, const XML_Char *el, const XML_Char *attr[])
{
  ...
  case IN_PAGE_REVISION:
    ...
    if (!strcmp(el, "timestamp"))
    {
      p->status = IN_PAGE_REVISION_TIMESTAMP;
      utstring_new(p->timestamp);
    }
    ...
    break;
  case IN_PAGE_REVISION_TIMESTAMP:
...
end(void *user_data, const XML_Char *el)
{
  ...
  case IN_PAGE_REVISION_TEXT:
    if (!strcmp(el, "text"))
    {
      ...
      {
        p->func(p->env, utstring_body(p->title), utstring_body(p->body), utstring_body(p->timestamp));
      }
      utstring_free(p->title);
      utstring_free(p->body);
      utstring_free(p->timestamp);
      p->title = NULL;
      p->body = NULL;
      p->timestamp = NULL;
      p->article_count++;
...
element_data(void *user_data, const XML_Char *data, int data_size)
{
  ...
  case IN_PAGE_TITLE:
    ...
    break;
  case IN_PAGE_REVISION_TIMESTAMP:
    utstring_bincpy(p->timestamp, data, data_size);
    break;
  case IN_PAGE_REVISION_TEXT:
